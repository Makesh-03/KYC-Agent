...
def kyc_multi_verify(files, expected_address, provider):
    ...
    score = sum(semantic_match(a1, a2) for i, a1 in enumerate(extracted_addresses) for j, a2 in enumerate(extracted_addresses) if i < j)
    ...
    status_msg = f"<div style='color:green;font-size:20px'><b>✅ Address Match Successful ({round(consistency_score*100)}%)</b></div>" if final_result else \
...
with gr.Blocks(css=custom_css, title="EZOFIS KYC Agent") as iface:
    ...
    verify_btn.click(fn=kyc_multi_verify, inputs=[file_inputs, expected_address, model_choice],
